...
import pandas as pd
game = pd.read_csv('Desktop/vgsales.csv')

games = list(game.groupby('Publisher'))

MediaPlataform = game.groupby('Platform').Global_Sales.mean()
...
MediaPlataform = game.groupby('Platform').Global_Sales.mean().head().plot.barh()
MedianaPlataform = game.groupby('Platform').Global_Sales.median().head().plot.barh()
company = game.groupby('Platform')['Global_Sales'].value_counts()
standarddeviation = company.std()

game["Year"].plot.hist(bins=30, edgecolor='black')
mediaPublic = game.groupby('Publisher').Global_Sales.mean().head().plot.barh()
medianPublic = game.groupby('Publisher').Global_Sales.median().head().plot.barh()
...
game = pd.read_csv('Desktop/vgsales.csv', sep=',', encoding='utf-8')
...
plt.title('Games Clusters and Centroids')
plt.xlabel('X')
plt.ylabel('Y')
plt.legend()
